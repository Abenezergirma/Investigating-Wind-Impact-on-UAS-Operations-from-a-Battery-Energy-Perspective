...
for fault_idx = 1:1
    ...
    if (fault_idx - 1 < 9) && ~(fault_idx == 1)
        ...
        is_full_loss = 1;
    elseif fault_idx - 1 < 17 && ~(fault_idx == 1)
        partial_percent=0.5 + 0.4*rand;
        motormult(motor_idx)=partial_percent;
    elseif ~(fault_idx == 1)
        saturation_num = (5500 - 1)*rand + 1;
        motormult(motor_idx)=0;
...
writetable(results_table, 'single_experiment.csv', 'Delimiter', ',');
